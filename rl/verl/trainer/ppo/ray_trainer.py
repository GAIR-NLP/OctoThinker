...
def _compute_response_info(batch):
    ...
    correct_response_length = response_length[sequence_score==1]
    return dict(
...
class RayPPOTrainer(object):
    ...
    def _create_dataloader(self, template_type=None):
    ...
    def _save_samples(self, batch: DataProto, split: str):
        ...
        prompts=[prompt.split("\nuser\n")[-1].split("\nassistant\n")[0].split("automatically by the \\boxed{} tag.\n")[-1].split("\nAssistant:")[0] for prompt in prompts]
        ...
        if split=='train': self.train_dataset.update_prompt_profile(prompts, rewards)
        data=[]
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            if epoch==1 and self.config.data.template_type=="icl_base":
                self._create_dataloader(template_type="cpt_base")
            for batch_dict in self.train_dataloader:
                ...
                self.global_steps += 1
            if self.config.data.dynamic_remove_easy_sample:
